...
func TestInitialize(t *testing.T) {
	...
	hallRequestAssigner(&system, HallRequestsForAllElevators, []string{"1", "2", "3"})
	t.Logf("system json: %s", encodeElevatorSystem(&system))
	transition := CheckOrderTransitionStatusForHallRequests(&system, HallRequestsForAllElevators, hallUp, 2, []string{"1", "2", "3"})
